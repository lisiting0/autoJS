...
async function test() {
    try {
        ...
        const muApp = new mu_app_1.MUApp();
        const result = await muApp.CheckIn({ passengerName: "杨展", ticketNo: "781-2470052743" });
        console.log(JSON.stringify(result));
    }
    catch (error) {
        console.log(error);
    ...
    */
    async CheckIn(input) {
        ...
        try {
            ...
            // 选座或者更换座位
            let xzBtn = await selector.findAllById("pbFunctionBtn", 100, 3000);
            if (xzBtn.length == 1) {
                xzBtn[0].click();
            }
            else {
            ...
            this.log("点击更换座位");
            // 选座页面 
            let lineSeats = await selector.findAllById("lin_seat", 100, 1000);
            if (lineSeats.length > 0) {
                console.log(`座位${lineSeats.length}排`);
                let clickSeat = false;
                for (const linseat of lineSeats) {
                    const seatImg = linseat.children.filter((x) => x.className === "android.widget.ImageView");
                    if (seatImg.length > 0) {
                        seatImg[0].click();
                        await (0, lang_1.delay)(700);
                        // 查看是否有座位号
                ...
                throw new Error("选座失败，请检查信息或者网络是否有效");
            }
            // 勾选
            ...
            if ((state === null || state === void 0 ? void 0 : state.text) === "办理成功") {
