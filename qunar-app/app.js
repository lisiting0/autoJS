...
const MAIN_PAGE_MINE_ID = "atom_alexhome_mod_usercenter"; // 我 页面图标
const MAIN_PAGE_HOME_ID = "atom_alexhome_mod_main"; // 主页页面图标
...
module.exports = {
  ...
  home: function () {
    log("前往主页");
    this.getById("atom_alexhome_tab_indicator", 200, 30000);
    let btn = this.getById(MAIN_PAGE_HOME_ID, 200, 30000);
    btn.click();
    log("主页已打开");
  },
  // 回滚到顶部
  ...
  // 检测是否有机器人聊天窗口
  checkNotificationLayout: function () {
    let notificaionLayoutId = this.waitForId(
      "pub_imsdk_notification_layout",
      50,
      2000
    );
    if (notificaionLayoutId) {
      sleep(5000);
    }
  },
};
